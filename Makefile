...
rootfs: hooks
	...
	fakeroot -- tar --numeric-owner --xattrs --acls --exclude-from=exclude -C $(BUILDDIR) -c . -f artixlinux.tar
	rm -rf $(BUILDDIR) alpm-hooks

docker-image: rootfs
	docker build -t $(DOCKER_ORGANIZATION)/$(DOCKER_IMAGE) .
	docker build -t $(DOCKER_ORGANIZATION)/$(DOCKER_IMAGE_OPENRC) ./openrc
	docker build -t $(DOCKER_ORGANIZATION)/$(DOCKER_IMAGE_RUNIT) ./runit
...
docker-image-test: docker-image
	...
	! docker run --rm $(DOCKER_ORGANIZATION)/$(DOCKER_IMAGE) pacman-key --lsign-key cromer@artixlinux.org
